...
const CircuitSmashersJudge = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (status === 'marking') {
        ...
        const criteria = await axiosInstance.get(API_PATHS.CIRCUIT_SMASHERS.GET_CRITERIA);
  ...
  const handleDownload = async (schoolId) => {
    try {
      const response = await axiosInstance.get(API_PATHS.CIRCUIT_SMASHERS.GET_RESOURCE(schoolId), {
